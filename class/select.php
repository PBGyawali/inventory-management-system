<?php
	...
	function Currency_list($value=null)
	{
		$html = '
		<select name="company_currency" id="company_currency" class="form-control" required>
			<option value="">Select Currency</option>
		';
		$datas = $this->currency_array();			
		$selected_value = $value;// value from database	
		foreach($datas as $data)
		{	$selected = ($selected_value == $data["code"]) ? "selected" : "";
			$html .= '<option value="'.$data["code"].'***'.$data["symbol"].'"'. $selected.'>'.$data["name"].'</option>';
		}
		$html .= '</select>';
		return $html;
	}

	function Timezone_list($value=null)
	{
		$timezones = array(
		...
		$html = '
		<select name="company_timezone" id="company_timezone" class="form-control" required>
