...
if(isset($_POST["company_name"]))
{
	...
	{	$password=$ims->clean_input($_POST["admin_password"]);
		$master_user_data = array(
									':user_name'				=>$ims->clean_input	($_POST["admin_email"]),
									':user_email'				=>$ims->clean_input	($_POST["admin_email"]),
									':user_password'			=>	password_hash($password, PASSWORD_DEFAULT),
									':user_type'				=>	'master',
								);
		$ims->query = "	INSERT INTO user 	(user_name, user_email, user_password, user_type) 
		VALUES (:user_name,  :user_email, :user_password, :user_type)	";

		$ims->execute($master_user_data);
		if($ims->row()>0)
			{
				$_SESSION['setup']='';
